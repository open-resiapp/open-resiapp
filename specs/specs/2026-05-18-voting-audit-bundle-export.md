...
- `votes.json` — the array of **per-item** vote leaves (multi-item votings, BYT-20260609-008) in canonical sort order `(recordedAt ASC, itemId, ballotId tiebreak)`. Each leaf is the pinned schema: `{votingId, itemId, unitId, ownerId, share: {num, den}, choice, recordedAt, voteType, confirmationKind, ballotHash, paperPhotoShas: [...]}`. `ballotHash` ties every item leaf to the one signed ballot it was submitted under; `paperPhotoShas` is the array of photo hashes for a paper ballot (one ballot may carry several photos).
- `result.json` — engine output in rational form (see "Result rationalization" below), as an **array of per-item results** (multi-item votings). Each item result carries: `itemId`, `quorumType`, `za`, `proti`, `zdrzalSa`, `total`, `zaPercent`, `protiPercent`, `zdrzalSaPercent` (all `{num, den}`), `passed`, `quorumReached`, `totalPossibleWeight: {num, den}`, `unitBreakdowns[]` (per-unit resolution with shares and rationale). `quorumType` is per item, not per voting.
...
2. **Merkle tree shape** — RFC 6962 binary merkle (Certificate Transparency log format), leaf prefix `0x00`, internal prefix `0x01`, sorted `(recordedAt ASC, itemId, ballotId tiebreak)` over the per-item leaves (multi-item votings, BYT-20260609-008). Inclusion proofs supported so a single voter can prove their item-vote is in the bundle without disclosing other ballots.
